...
SRC="$ROOT/data/prepared/modelzip_work/tests/eng-zho_Hans/cmp32.eng-zho_Hans.eng"
REF="$ROOT/data/prepared/modelzip_work/tests/eng-zho_Hans/cmp32.eng-zho_Hans.zho_Hans"
run_one() {
  local id="$1" model="$2" codec="${3:-}"
  "$ROOT/scripts/run_with_gpu_trace.sh" "direct_$id" "$PY" "$SCRIPT" --system-id "$id" --model "$model" --input "$SRC" --reference "$REF" --output "$ROOT/results/predictions/${id}.cmp32.txt" --result "$ROOT/results/metrics/direct_${id}.json" --batch-size 8 $codec
}
run_one baseline-bf16 "$ROOT/artifacts/models/cache/google/gemma-3-12b-it"
run_one organizer-bnb-q8 "$ROOT/submissions/bnb-q8/workdir/model"
